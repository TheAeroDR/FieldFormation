...
#include <filesystem>
#include <chrono>
#include "sensitivity_parameters.h"

// Global sensitivity parameters
SensitivityParams g_sensitivity_params;
...
struct SimpleParticle {
    ...
    double charge_profile() {
        double q;
        double ed = 2 * g_sensitivity_params.electron_density_bias;
        if(ed == 1){
            ...
            // Interpolate for non-standard ed values
            q = 12.53 * ed * std::pow(r, 2) - 2.732e4 * ed * std::pow(r, -0.8);
        }
        return q;
    }
...
double rad2height(double D) {
    return g_sensitivity_params.height_bias * std::pow((D / 2.0) * (1.0 / 0.913), 2.0);
}
...
double dd_rad_profile(double D) {
    ...
    rand_val = std::pow(rand_val, g_sensitivity_params.rad_profile_bias);
    
    return (D / 2.0) * (1.0 - 0.5 * rand_val);
}

std::vector<double> thirdbiased(int N, double min, double max) {
    double power = std::log(g_sensitivity_params.height_median_bias)/ std::log(0.5);
...
std::vector<SimpleParticle> initializeParticles(double D, int N_Sim, int N_AtHeight, unsigned int seed) {
    const double dust_rho = 2.0e6 * g_sensitivity_params.dust_density_bias;
    const double dust_loading = 0.296 * g_sensitivity_params.dust_loading_bias;

    double max_height = rad2height(D);
    double volume = dd_volume(D, max_height);
    double V0 = tan_vel_profile(max_height) * g_sensitivity_params.eyewall_velocity_bias;
    double dust_mass = dust_loading * volume;
    double av_mass = dust_rho * (4.0 / 3.0) * PhysicalConstants::pi * std::pow(3e-6 * g_sensitivity_params.avg_particle_size_bias, 3.0);
    ...
    std::uniform_real_distribution<> angle_dist(0.0, 2.0 * PhysicalConstants::pi* g_sensitivity_params.theta_bias);
    ...
    for (int i = 0; i < N_heights; ++i) {
        ...
        for (int j = 0; j < N_AtHeight; ++j) {
            ...
            p.z = heights[i];
            p.r = g_sensitivity_params.diameter_bias * diameters[j] / 2.0;
            ...
            p.q = -1.0 * clump_factor * p.charge_profile() * g_sensitivity_params.charge_bias;
...
void writeTimeseriesFile(const ConvergenceResult& result, const std::string& output_suffix = "") {
    std::string filename = "sensitivity" + std::to_string(result.particle_count) + "_timeseries" + output_suffix + ".csv";
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::string test_name = "baseline";
        std::string output_suffix = "";
        
        if (argc >= 2) {
            test_name = std::string(argv[1]);
            output_suffix = "_" + test_name;
        }
        
        // Set sensitivity parameters
        g_sensitivity_params = SensitivityParams(test_name);
        
        std::cout << "CUDA Sensitivity Study: " << test_name << std::endl;
        std::cout << g_sensitivity_params.getDescription() << std::endl;
        // Check CUDA device
        int device_count;
        ...
        std::cout << "CUDA Sensitivity Study Starting..." << std::endl;
        
        // Parameters
        double D = 7.0;
        int N_AtHeight = 100 * g_sensitivity_params.particles_per_height_bias;
        double center_x = -102.0, center_y = 0.0;
        double velocity_x = 5.38 * g_sensitivity_params.translational_velocity_bias; 
        double velocity_y = 0.0;
        
        // Test point (0, 0, 1)
        double test_x = 0.0, test_y = 0.0, test_z = 1.0;
        
        std::vector<int> particle_counts = {100000};
        int num_runs = 1000;
        ...
        for (int N_Sim : particle_counts) {
            ...
            writeTimeseriesFile(result, output_suffix);
        ...
        std::cout << "\nSensitivity study completed!" << std::endl;
