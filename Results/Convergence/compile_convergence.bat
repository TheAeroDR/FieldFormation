...
echo Compiling CUDA Convergence Study...
nvcc -O3 -arch=sm_89 -std=c++17 convergence_cuda.cu -o convergence_cuda.exe

if %ERRORLEVEL% EQU 0 (
    echo.
    echo ===== COMPILATION SUCCESSFUL =====
    echo Executable: onvergence_cuda.exe
    echo Double precision enabled for accurate small magnetic field values
    echo --use_fast_math removed to preserve precision
    echo.
    echo To run: convergence.exe
    echo.
) else (
    echo.
